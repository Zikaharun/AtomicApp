...
if (isset($_POST['id_habit_progress'])) {
    $_SESSION['id_habit_progress'] = $_POST['id_habit_progress'];
...
$id_habit_progress = isset($_SESSION['id_habit_progress']) ? $_SESSION['id_habit_progress'] : (isset($_GET['id_habit_progress']) ? intval($_GET['id_habit_progress']) : null);

// Jika id_habit_progress tidak tersedia, redirect ke halaman lain atau tampilkan pesan error
if ($id_habit_progress === null) {
    header("Location: ../goodhabits_progress/goodhabits_progress_tracker.php"); // Ganti dengan halaman yang sesuai
    exit;
}

// Query untuk mendapatkan detail progress habit
$stmt_progress = $connect->prepare("SELECT hp.*, h.habit_name FROM habit_progress hp JOIN habit h ON hp.id_habit = h.id_habit WHERE hp.id_habit_progress = ?");
$stmt_progress->bind_param("i", $id_habit_progress);
...
$stmt_weekly = $connect->prepare("
    ...
    FROM habit_progress 
    WHERE id_habit = ? AND date >= DATE_SUB(NOW(), INTERVAL 1 WEEK)
    GROUP BY date
    ORDER BY date ASC
");
$stmt_weekly->bind_param("i", $progress_result['id_habit']);
$stmt_weekly->execute();
...
$stmt_daily = $connect->prepare("
    SELECT date, SUM(daily_frequency) AS total_daily_frequency 
    FROM habit_progress 
    WHERE id_habit = ? AND date >= DATE_SUB(NOW(), INTERVAL 30 DAY)
    GROUP BY date
    ORDER BY date ASC;
");

$stmt_daily->bind_param("i", $progress_result['id_habit']);
$stmt_daily->execute();
$daily_data = $stmt_daily->get_result()->fetch_all(MYSQLI_ASSOC);

// Query untuk mendapatkan total progres harian
$stmt_total_daily = $connect->prepare('SELECT SUM(daily_frequency) AS total_progress_daily FROM habit_progress WHERE id_habit = ?');
$stmt_total_daily->bind_param('i', $progress_result['id_habit']);
